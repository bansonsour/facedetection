package org.dp.facedetection.util.camera;
...
//                parameters.set("orientation", "portrait");
    ...
    private void setOptimalPreviewSize(Camera.Parameters cameraParams, int targetWidth, int targetHeight) {
        Camera.Size optimalSize = getOptimalPreviewSize(mCamera, targetWidth, targetHeight);


        float preview_width = (float) optimalSize.width;
        float preview_height = (float) optimalSize.height;
        float previewRatio = preview_width / preview_height;
        int surface_width=0;
        int surface_height=0;
        if (this.previewDisplayView != null) {


            if (this.sw > this.sh) {//横屏
                surface_width = (int) ((float) this.sh * previewRatio);
                surface_height = this.sh;
                /*surface_width = this.sw;
                surface_height = this.sw/pre_rate;*/
            } else {//竖屏
                surface_width = this.sw;
                surface_height = (int) ((float) this.sw * previewRatio);
                /*surface_width = this.sh/pre_rate;
                surface_height = this.sh;*/
            }

            previewDisplayView.getLayoutParams().width = surface_height;
            previewDisplayView.getLayoutParams().height = surface_width;
